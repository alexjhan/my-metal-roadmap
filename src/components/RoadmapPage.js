...
export default function RoadmapPage() {
  ...
  const handleShowVersion = (version) => {
    console.log('handleShowVersion called with:', version);
    
    if (version && version.nodes && version.edges) {
      // Asegurar que la versión tenga la estructura correcta
      const versionData = {
        nodes: version.nodes,
        edges: version.edges,
        lastModified: version.created_at || new Date().toISOString(),
        versionId: version.id,
        versionInfo: version
      };
      
      console.log('Setting currentVersion to:', versionData);
      setCurrentVersion(versionData);
    } else {
      console.error('Version data is invalid:', version);
    }
  };
    ...
    <>
      ...
      >
        ...
        */}
        ...
        {/* Indicador de versión específica cargada */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-blue-700 font-medium">
                  Mostrando versión específica (ID: {currentVersion.versionId})
                </span>
              </div>
              <button
                onClick={() => {
                ...
                title="Volver a versión por defecto"
              >
                Restablecer
              </button>
            </div>
